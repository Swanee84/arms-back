...
class UserService {
	async selUserList(academyId: number, branchId?: number, role?: string, email?: string, name?: string, phoneNo?: string): Promise<IResponse> {
		const whereObj: { academyId: number; branchId?: number } = { academyId };
		if (branchId) {
			whereObj.branchId = branchId;
		}
		const userWhereObj: { role?: string; email?: Record<string, string>; name?: Record<string, string>; phoneNo?: Record<string, string> } = {};
		if (role) {
	...
	async selUserDetail(serachRole: string, userId: number): Promise<IResponse> {
		...
		} else if (role === RoleConst.TEACHER || role === RoleConst.DIRECTOR || role === RoleConst.PRESIDENT) {
			// 선생님이면
